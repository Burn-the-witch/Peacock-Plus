...
module.exports = async function (controller) {
	...
	for (const id of ESCALATIONS_TO_CONVERT) {
		...
		contract.Metadata.GroupDefinition.Order = contract.Metadata.GroupDefinition.Order.filter((_, index) =>
		...
		for (const EscalationLevel of contractsToRename) {
			...
			actualcontract.Metadata.Title = actualcontract.Metadata.Title.replace(/3/g, "2").replace(/5/g, "3")
